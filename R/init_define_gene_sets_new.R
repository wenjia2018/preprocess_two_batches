init_define_gene_sets_new <- function(expression_fname){ 
  ...
  ses_de_up <-(read_excel(str_c(path,"/share/ext/data/de_ses_up_down.xlsx"), sheet = 1, col_names = T))$Gene %>% intersect(gene_signature)
  ses_de_down <-(read_excel(str_c(path,"/share/ext/data/de_ses_up_down.xlsx"), sheet = 2, col_names = T))$Gene %>% intersect(gene_signature)
  
  g1 = c("RPL35A", "RPS29", "RPS18", "RPS14", "RPS17", "RPS15A", "RPL39", "RPS21", "RPL35", 
         "RPL26", "RPL31", "RPL23", "RPL11", "RPL41") %>% intersect(gene_signature)
  g2 = c("CEBPE", "RPS18", "RPL39", "DSC2") %>% intersect(gene_signature)
  ...
  outcome_variables <- c(
    ...
    "Depression",
    "CKD", "CKD_up", "CKD_down", "ses_de_up", "ses_de_down", "g1", "g2"
  )
